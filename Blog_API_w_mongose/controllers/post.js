...
exports.getPosts = async(req,res)=>{
   ...
   // it is like a "JOIN" u just want to get some property not all
    const data = axios Post.find().populate('category', 'name')
    res.status(200).json({
        succes:true,
        data:data,
        count : data.length
...
exports.putPost = async(req,res)=>{
    ...
    const data = await Post.findAnUpdate(req.params.id, req.body,{new:true, runvalidators:true})
    res.status(200).json({
        succes:true,
        data:data
    })
...
exports.deletePost = async (req,res)=>{
    const data = await Post.findByIdAndDelete(req.parasms.id)
    res.status(204).json({
        succes:true,
        data:{}
    })
